...
const createMessage = (id: string, senderId: number | 'me', content: string, type: MessageType, minutesAgo: number, status: MessageStatus): Message => ({
    id,
    senderId,
    content,
    type,
...
export const mockConversationsData: Conversation[] = mockUsers.slice(0, 8).map((user, index) => ({
    ...
    messages: [
        ...
        ...(index % 3 === 1 ? [createMessage(`${user.id}-5`, user.id, `I'm into photography. Here's one of my recent clicks!`, MessageType.TEXT, 8, MessageStatus.READ), createMessage(`${user.id}-6`, user.id, `https://picsum.photos/400/300?random=${user.id+20}`, MessageType.IMAGE, 7, MessageStatus.READ)] : []),
    ]
}));