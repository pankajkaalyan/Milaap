    ...
    (data) => {
        const finalProfileData: Partial<UserProfile> = {
            ...
            partnerPreferences: {
                ...
                castes: data.partnerPreferences?.castes?.split(',').map(s => s.trim()).filter(Boolean),
                professions: data.partnerPreferences?.professions?.split(',').map(s => s.trim()).filter(Boolean),
            }
        };
  ...
  const handlePhotoAdd = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
        ...
        if(currentPhotos.length < 5) {
            setFieldValue('photos', [...currentPhotos, newPhotoUrl]);
        } else {
